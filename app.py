...
with tab3:
    ...
    # Chart 1: Brand Distribution Pie - Simple Market Share Insight
    if 'Brand' in df_sample.columns:
        fig_brand_pie = px.pie(df_sample, names='Brand', title="EV Brand Market Shares (Dataset Snapshot)", hole=0.4)
        fig_brand_pie.update_traces(textposition='inside', textinfo='percent+label')
        fig_brand_pie.update_layout(height=400)
